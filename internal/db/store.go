...
func (s *Store) EnsureUser(ctx context.Context, uid, display string) error {
	_, err := s.Pool.Exec(ctx, `
	  ...
	  VALUES ($1,$2)
	  ON CONFLICT (uid) DO NOTHING`, uid, display)
	return err
}



func (s *Store) ListUserSongs(ctx context.Context, uid string) ([]map[string]any, error) {
...
func (s *Store) ListPublicSongs(ctx context.Context, limit int) ([]map[string]any, error) {
	rows, err := s.Pool.Query(ctx, `
		SELECT id,title,is_public,payload,updated_at
		  FROM songs
...
func scanSong(row pgx.Row) (Song, error) {
	var s Song
	err := row.Scan(&s.ID, &s.OwnerUID, &s.Title, &s.IsPublic,
